        ...
        <View style={{ flex: 1 }}>
          <View
            ...
            <View
              ...
              <TextInput
                ...
                onChangeText={(text) => setWhatArray(candidate.jobs.filter((item) => item.Title.toLowerCase().includes(text.toLowerCase())))}
              ></TextInput>

              <TextInput
                placeholder="Where"
                style={[styles.textinput, { marginRight: 20 }]}
                onChangeText={(text) => setWhereArray(candidate.jobs.filter((item) => item.Location.toLowerCase().includes(text.toLowerCase())))}
              ></TextInput>
            </View>
            <TouchableOpacity
              style={styles.btn}
          ...
          <View
            ...
            <TouchableOpacity
              style={styles.btn}
              onPress={() => {
                var filter = salaryRangeArray.filter((item) => qualificationTypeArray.includes(item))
                applyJobFilter(filter.filter((item) => experienceTypeArray.includes(item)))
                navigation.navigate('BrowseJobs');
              }}>
              <Text style={styles.btntext}>Search</Text>
            </TouchableOpacity>
