...
import { View, Text, StyleSheet, TouchableOpacity, Image, TextInput, ActivityIndicator, Modal, Pressable } from 'react-native';
...
import { markFavorite, sendInvite, getCandidateProfile } from '../redux/actions/employer'
import DatePicker from 'react-native-date-picker';

const CProfile = ({ navigation, employer, markFavorite, sendInvite, getCandidateProfile }) => {
    ...
    return (
        <Container style={{ backgroundColor: 'white' }} >
            ...
            <Content >
                ...
                {employer.candidateProfile != null ? <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flex: 1, flexDirection: 'row' }}>
                        <TouchableOpacity disabled={sentInvite || employer.candidateProfile.InInvited} onPress={() => setModalVisible(true)}>
                            <View style={styles.textinput}>
                                <Text style={{ textAlign: 'center', marginTop: 5 }} >{sentInvite || employer.candidateProfile.InInvited ? "Invited" : "Send Invite"}</Text>
                            </View>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => markFavorite(employer.candidateProfile.Profile)}>
                ...
                </View> : <ActivityIndicator size={'large'} color={'#009961'} style={{ justifyContent: 'center', alignItems: 'center' }} />}
            </Content>
            <Modal
                animationType="slide"
                transparent={true}
                    ...
                    <View style={styles.modalView}>
                        ...
                        <View style={{ flex: 1, flexDirection: 'row', marginTop: 10, justifyContent: 'space-between' }}>
                            <Pressable
                                ...
                                onPress={() => {
                                    setSentInvite(true);
                                    sendInvite(employer.candidateProfile.Profile.CandidateId, datename)
                                    setModalVisible(!modalVisible)
                                }}
                            >
