...
export const browseCandidate = () => {
  return (dispatch, store) => {
    ...
    axios
      ...
      .then((response) => {
        dispatch({
          type: types.EMPLOYER_CANDIDATES_FETCHED,
...
export const sendInvite = (id, time) => {
  return (dispatch, store) => {
    ...
    axios
      .post(
        BASE_URL + '/api/employer/SendInvite',
        {
          candidateid: id,
          DateTime: time,
        },
...
export const getFavorites = () => {
  return (dispatch, store) => {
    ...
    axios
      ...
      .then((response) => {
        dispatch({
          type: types.EMPLOYER_FAVORITES_FETCHED,
          favorites: response.data.favourite,
        });
      })
      .catch((error) => {
        dispatch({type: types.EMPLOYER_ERROR, message: error.message});
...
export const markFavorite = (job) => {
  return (dispatch, store) => {
    ...
    axios
      .get(
        BASE_URL + '/api/employer/Favourite?candidateid=' + job.CandidateId,
        {
          headers: {
            Authorization: 'bearer ' + store().auth.token,
            // Authorization: "Bearer ImR3jcyncQexl3290Mbdco9hCSbbyfC5u8cxii1mU85o_4ekExt3w7jxLjGxug67hfXDfGxlkPJBNpssd327Cbs7N7MJdNNmEeeWtOm5xuCdIJBDNxXT-OvzEycuQBxFr1CpZl3iQL2tTlofPVrs42Y25emZuEerwDstlwnjiA-stovcLA3P0qQK4to9n_WueBoXGoNUvdcmt6y74AAXXh2QhleVZ3WBrJaycGZmmyx-seyeRCPoP36kEdRz9_Dhap-K_5_SCIIVGuPY8Pa3PWTXmjDGCUQhhIHOacNvtpxqVdErnM9Mo93q9alesbzd0xvML-pyKfcIhthFUSl-6V9dPvLQLvRmkus0Bn_WM4uBuDUjwBmTfohfESP_1ZetSLLr3CzKoMr-dZSiisAz4WA9hRCR6XAbvLRaop0bZDqwypPVNPq6UaaRgZuDdafTLwCAX-4Swx_nn47oMdLjH4NaUnn5nAiFnqZqHwiSYfwOT_e9GQ_aVMqUyJkqtWiIHp0DS_jS-ERmvUAIMYHMb6aVPv6V3t9H8w_89ox5FJpaQidjbVL9N48v0s7ujguT"
      ...
      .then((response) => {
        if (
          store().employer.favorites.filter(
            (item) => item.CandidateId == job.CandidateId,
          ).length == 0
        ) {
          console.log('Working');
          dispatch({
            type: types.EMPLOYER_FAVORITES_FETCHED,
            favorites: store().employer.favorites.push(job),
          });
        } else {
          dispatch({
            type: types.EMPLOYER_FAVORITES_FETCHED,
            favorites: store().employer.favorites.filter(
              (item) => item.CandidateId != job.CandidateId,
            ),
          });
        }
      ...
      .then((response) => {
        ...
        if (response.data.result == 'success') {
          axios
            ...
            .then((response) => {
              dispatch({
                type: types.EMPLOYER_JOBS_FETCHED,
...
export const getHiredCandidates = () => {
  return (dispatch, store) => {
    ...
    axios
      ...
      .then((response) => {
        dispatch({
          type: types.EMPLOYER_HIRED_CANDIDATES_FETCHED,
...
export const updateCompanyDetail = (
  ...
  return (dispatch, store) => {
    ...
    axios
      .post(
        BASE_URL + '/api/employer/updateprofile',
        {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.result == 'success') {
          dispatch({
            type: types.EMPLOYER_JOBS_FETCHED,
            jobs: store().employer.jobs.filter((item) => item.Id != id),
          });
        } else {
  ...
  return (dispatch, store) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        dispatch({
          type: types.CANDIDATE_INVITATION_FETCHED,
          invitations: store().candidate.invitations.filter(
            (item) => item.InviteId != id,
          ),
        });
        // alert(id)
        //dispatch({ type: types.EMPLOYER_CANDIDATES_FETCHED, candidates: response.data.candidates })
...
export const rescheduleInvite = (id, time) => {
  return (dispatch, store) => {
    ...
    axios
      .post(
        BASE_URL + '/api/employer/reschedule',
        {
          candidateid: id,
          DateTime: time,
        },
...
export const applyCandidateFilter = (candidates) => {
  return (dispatch) => {
    ...
    dispatch({
      type: types.EMPLOYER_APPLY_CANDIDATE_FILTER,
